...
describe('Starting tests for AngularApp', function() {
  ...
  it('network-name should be payment-network@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('payment-network@0.0.1.bna');
    });
  });

  it('navbar-brand should be AngularApp',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('AngularApp');
    });
  });
